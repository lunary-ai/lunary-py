...
try:
    ...
    def _parse_output(raw_output: dict) -> Any:
        ...
        if isinstance(raw_output, list):
            return [_parse_output(output) for output in raw_output]

        if isinstance(raw_output, BaseMessage) or isinstance(raw_output, BaseMessageChunk):
            return _parse_lc_message(raw_output)
        ...
        if lc_serialized_messages: 
            ...
            for message in lc_serialized_messages:
                if _is_serialized_lc_message(message):
                    ...
                    messages.append(_parse_lc_message(message))
            return messages
    ...
    def _is_serialized_lc_message(obj: dict) -> bool:
        # Replace by langchain Serializable.get_lc_namespace
        try: 
            if obj['lc'] != 1:
                return False
    ...
    class LunaryCallbackHandler(BaseCallbackHandler):
        ...
        def on_agent_action(self, action, *, run_id, parent_run_id: UUID | None = None, **kwargs ) -> Any:
            print('\n\n\n\n')
            print('------')
            print("AGENT_ACTION")
            print(action)
